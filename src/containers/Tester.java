...
public class Tester<C> {
	...
	public static <C> void run(C container,List<Test<C>> tests){
		new Tester<C>(container, tests).timeTester();
	}

	public static <C> void run(C container,List<Test<C>> tests,TestParam[] paramList){
		new Tester<C>(container, tests, paramList).timeTester();
	}
	
	private void displayHeader(){
		//calculate with and head with "-"
	...
	public void timeTester() {
		...
		for(TestParam param:paramList){
			...
			for(Test<C> test:tests){
				...
				long timePerRep = duration/reps;  //Nanosecond  纳秒
